...
- **T18 — server half DONE + VERIFIED** (2026-07-17, orchestrator; commit pending with client sweep). Migration `0005_block_invisibility.sql`: bidirectional blocked-pair exclusion inlined into the DEFINER objects `public_profiles` (D13 widening — audit F1b), `attendee_previews` (audit F1), and `nearby_events` (INVOKER→DEFINER per D15). Mechanism + why-not-a-helper logged in **D15**. Hostile suite extended (users C/D, A→C block exercising the predicate's 2nd leg, reverse-viewer-as-C section proving the definer read — not caller RLS — drives exclusion) and run green on a fresh 0001→0005 stack: `ALL HOSTILE-USER TESTS PASSED`, every prior Phase 1/2 assertion still passing. The live audit gap F1/F1b is closed at the data layer (server returns no blocked-user data in either direction regardless of client). **Communities deferred to Phase 3 T26** (D15 — no Phase 2 definer read path).
- **T19 — APPROVED** (2026-07-18, swiftui-builder/Opus; Loop 3 by orchestrator). Safety surfaces: `Core/Security/EmergencyContactStore.swift` (Keychain-only, reuses `KeychainTokenStore`'s generic-password primitives → inherits `WhenUnlockedThisDeviceOnly` verbatim), `Features/Safety/{FirstMeetingSafetySheet,PanicButton,EmergencyContactView}.swift`, wired into EventDetail + a Settings→Safety row. **D9 independently verified by orchestrator**: the only `EmergencyContact`/`emergency_contact` references are the store + the three Safety views + the Settings row; the one network-pattern grep hit is a *comment* defining what a D9 violation would be — the store imports Foundation only and does pure Keychain ops. First-meeting trigger is server-derived (`ownActiveTickets().isEmpty`, NOT UserDefaults — survives reinstall), non-dismissable sheet gates the first RSVP; panic button window-gated ±2h with `tel:`+`sms:` (contact is only the local SMS recipient; body carries public venue coords only). Scope clean (no Models/migration/function changes). Review-Notes §1.4.1 drafted. Build green; **116 tests, 0 failures** (6 skipped), SafetyTests 9/9. Ponytails: per-host first-meeting granularity (needs host linkage on tickets), `112` pan-regional emergency default (locale resolution is the upgrade), dial-prioritized-over-SMS. **This commit also lands T20's deferred Settings→About "© OpenStreetMap contributors" ODbL line.**
- **T20 — DONE (pipeline + attribution); hosted population pending deploy** (2026-07-18, orchestrator). About-screen ODbL line landed in the T19 commit. Pipeline details: `scripts/seed/seed.py` (Python 3 stdlib only): Overpass fetch per city (retry + mirror), OSM tag→`space_category` mapping, validation (name 1–120, coords), dedupe + idempotent upsert on `source_ref` (partial-unique index), curation to a per-city cap (D6: 100–200). D7 key-guard **runs before any network fetch** and refuses to run if the connection secret is in a git-tracked file (bug found + fixed in review: `git grep` from a subdir only searches that subtree — now resolves the repo root). Verified against the local stack: Bengaluru 2362 raw→150 validated; **idempotent load 25→25, 0 duplicates**; Mumbai fetch + secretless `--emit-sql` path both green; guard positive/negative both correct. `docs/compliance/attribution.md` (ODbL: attribution + share-alike boundary + Google-prohibited). **Deferred:** (a) hosted population (100–200/city) — needs the hosted deploy (link + service-role key local-only), a standing user action; (b) the Settings→About "© OpenStreetMap contributors" line — held until T19 lands to avoid a Settings-tree collision (T19 is editing it now).
- **T18 — APPROVED (client sweep + full task)** (2026-07-18, single commit with the server half). Client sweep (swiftui-builder/Opus, finished by orchestrator after the builder hit a session limit mid-test): `Core/BlockSignal.swift` — a narrow, **payload-free** `.thrdUserBlocked` NotificationCenter signal (the blocked id is never broadcast; observers re-fetch through the server-filtered repository, so the client never makes the filtering decision — correct for a safety feature). `EventDetailViewModel.blockHost()`/`ProfileViewModel.blockUser()` post it on success; `DiscoverViewModel` reloads on it (subscribed before the seam guard so block-safety never depends on the live-location path); `EventDetailViewModel` refetches the attendee strip (previews carry no ids → client-side filtering is impossible, refetch through 0005 is the only correct drop). Card drop animates with a reduce-motion fallback; a11y intact. Loop 3: scope matches delegation (no Models/migration/function changes). Build green; **107 tests, 0 failures** (6 integration skipped), incl. the new `testBlockSignalReloadsDiscoverAndDropsBlockedHostEvents`. Ponytail: a block reloads the whole nearby set (trivial at Phase 2 sizes; targeted single-row drop is the upgrade).
...
- **T17 — APPROVED** (2026-07-17, commit `e6b3a51` in the app repo). Loop 3 line-by-line: wire body is `{event_id, action}` only — no client user_id (Layer 5); the `ownActiveTickets()` read adds an **explicit `user_id` filter on top of RLS** so the host clause of `tickets_select_own_or_host` can't smear other attendees' tickets in as "your spot" (real leak averted) + explicit column list (rule 5); canonical `TicketStatus` reused, no redeclare (D10); error mapping non-leaking, 400 split on the stable slug (`event_not_open` surfaced, `invalid_action`→generic). Optimistic count math matches Artifact B §4 (increment only for `going`; waitlist never counts); double-submit guarded; server response authoritative on reconcile. UI Excellence honored (spring + reduce-motion fallback, success/error haptics, a11y label folds the badge in once). Live 8/8 against the real `rsvp_event` on a local stack; 106 tests green. **Two Phase-3 follow-ups (not fixes):** (1) paid events currently surface the `event_not_open` copy on tap rather than a pre-tap disabled/priced CTA — correct for Phase 2 (no paid events seeded; paid creation is Phase 3) but Phase 3's paid-ticketing task owns the real CTA state; (2) cross-screen "your spot" staleness (badges reflect last Discover load) — logged ponytail, a shared RSVP store is the Phase 3/4 upgrade. `403 verification_required` and live `429` are unit-covered (mapping + copy), not live-exercised — server behavior already proven in T16.
