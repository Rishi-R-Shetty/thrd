...
- **T15 — APPROVED w/ fix T15.1** (2026-07-14, commit `cb560ae` + fix pending). Detail screens, explicit column lists on every query (verified), attendee first-name-only proven on the wire, Report/Block re-mounted on host, 93/93. Fix T15.1 dispatched: both detail VMs mask a loaded section behind a single shared error flag when the *other* section's fetch fails — partial-failure correctness bug, per-section error flags. One reviewer finding refuted (duplicate-pin: nearby_spaces RPC is one-row-per-space, no fan-out).
